...
template<typename DATA_T,
         ...
         size_t MEM_WIDTH>
void
MMU_pack(hls::stream<DATA_T>& store_stream,
...
template<typename DATA_T,
         ...
         size_t MEM_WIDTH>
void
MMU_unpack(hls::stream<DATA_T>& load_stream,
           hls::stream<DDR_WORD_T>& packed_stream)
{
#pragma HLS pipeline II = 1
 ...
 */
template<typename DATA_T, size_t DATA_PER_WORD>
void
MMU_fast(hls::stream<DATA_T>& in_stream,
    ...
    switch (state) {
      ...
      case stall:
        if (in_stream.size() < DATA_PER_WORD) {
          if (out_stream.write_nb(buff_w)) {
            /* Out stream not full and packet not ready, return to bypass state
             */
            flagbuff_w = false;
            state = bypass;
            ddr_data = 0;
...
/**
 * Process handling the communication with the ddr.
...
/* hls::stream_globals::incr_task_counter(); */
    ...
    hls_thread_local hls::task mmu_pack(
      MMU_pack<DATA_T, DDR_WORD_T, DATA_WIDTH, DATA_PER_WORD, MEM_WIDTH>,
      store_stream,
      store_p_stream);

    hls_thread_local hls::task mmu_unpack(
      MMU_unpack<DATA_T, DDR_WORD_T, DATA_WIDTH, DATA_PER_WORD, MEM_WIDTH>,
      load_stream,
      load_p_stream);
}
